...
formCadUsuario.onsubmit = (evento) => {
    ...
    if(UsuaCtrl.validar(usuario,email,senha,senha_confirmacao,perfil)){
        ...
        if (UsuaCtrl.validarUsuario(usuario)) {
        ...
        //email
        if (!UsuaCtrl.validarEmail(email)) {
            emailInput.classList.remove('is-valid');
            emailInput.classList.add('is-invalid');
        }
        else if(UsuaCtrl.verificar(email)){
            emailInput.classList.remove('is-valid');
            emailInput.classList.add('is-invalid');
        ...
        //senha
        if(!UsuaCtrl.vazioSenha(senha) && !UsuaCtrl.vazioSenhaConfirmacao(senha_confirmacao)){
            if (UsuaCtrl.validarSenha(senha,senha_confirmacao)) {
            ...
            if(UsuaCtrl.vazioSenha(senha)){
                senhaInput.classList.remove('is-valid');
                senhaInput.classList.add('is-invalid');
            }
            if(UsuaCtrl.vazioSenhaConfirmacao(senha_confirmacao)){
                senhaConfirmacaoInput.classList.remove('is-valid');
                senhaConfirmacaoInput.classList.add('is-invalid');
            }
        }
        ...
        //perfil
        if (UsuaCtrl.validarPerfil(perfil)) {
...
function limparClassesInvalidas() {
    ...
    if(listaUsuarios.length > 0){
        ...
        for (let i = 0; i < listaUsuarios.length; i++){
            ...
            linha.innerHTML = `
                ...
                <td>
                    <button type='button' class='btn btn-danger' onclick='excluirUsuario("${listaUsuarios[i].email}")'>
